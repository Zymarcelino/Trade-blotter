/**
 * Shared domain types for the trade blotter backend.
 *
 * Single source of truth for the trade domain model, DTOs, pagination shapes,
 * API response wrappers, and the WebSocket message protocol. Follows the
 * domain-modelling steering: branded IDs, discriminated unions, const objects
 * (no enums), Readonly domain objects, and never uses `any`.
 */

/**
 * A nominal (branded) trade identifier in the format `TRD-XXXXXX`.
 * The phantom `__brand` makes it structurally distinct from a plain string.
 */
...
/** Pagination input parsed from the query string. */
...
/** A paginated slice of results returned by the service layer. */
export interface PaginatedResult<T> {
  data: T[];
  meta: PaginationMeta;
}

/** Input to {@link ITradeRepository.create}. */
export type NewTrade = Omit<Trade, 'id' | 'tradeDate' | 'status'>;

/** A single row to insert into the `trade_audit` table. */
